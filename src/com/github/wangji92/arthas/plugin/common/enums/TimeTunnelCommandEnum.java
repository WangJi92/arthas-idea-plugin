...
public enum TimeTunnelCommandEnum implements EnumCodeMsg<String> {
    ...
    GET_CONTEXT_TARGET_BEAN("tt -w '#beanName=\"commonController\",#targetBean=target.getApplicationContext().getBean(#beanName),@org.springframework.aop.support.AopUtils@getTargetClass(#targetBean)' -x 3 -i 1000", "通过目标Bean的非代理原始对象,首先通过tt获取到spring context 对象"),
